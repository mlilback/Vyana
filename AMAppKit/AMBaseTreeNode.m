...
#import "AMBaseTreeNode.h"
#import "NSArray+AMExtensions.h"

@implementation AMBaseTreeNode
@synthesize nodeTitle=_nodeTitle, nodeIcon=_nodeIcon, children=_children,
	selectable=_isSelectable, representedObject=_repObj;
...
{
	if (!_nodeIcon || ![_nodeIcon isEqual:icon]) {
		[_nodeIcon release];
		_nodeIcon = [icon retain];
	}
}
...
{
	return [self.nodeTitle caseInsensitiveCompare:aNode.nodeTitle];
}
